...
class API {
    ...
    constructor(client) {
        ...
        /**
		 * The amount of calls remaining before the rate limit is hit.
		 * @since 3.0.0
		 * @type {number}
		 * @private
		 */
        ...
        /**
		 * The time when the user can get some more number of calls to make for the api.
		 * @since 3.0.0
		 * @type {number}
		 * @private
		 */
    ...
    getKeyStats(options) {
        return this._get('auth/stats', options);
    }

    getEndpoints(options) {
        return this._get('endpoints', options);
    }
