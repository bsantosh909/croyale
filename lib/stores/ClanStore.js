...
class ClanStore extends Store {
     ...
     * @param {*} [options={}] Optional information for fetching the player data.
    ...
    async getTracking(tag) {
        ...
        return this.client.Api.getClanTracking(verifiedTag);
    }
     ...
     */
    async getTopClans() {
        const _rawData = await this.client.Api.getTopClans();
    ...
    async getPopularClans() {
        const _rawData = await this.client.Api.getPopularClans();
        return _rawData.map(_single => new Clan(_single));
    }
