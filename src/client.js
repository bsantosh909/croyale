...
class Client {
    ...
    async getPlayer(tag, options = {}) {
        ...
        let verifiedTag = this.verifyTag(tag);
        ...
        let { body } = await get(`http://api.cr-api.com/player/${verifiedTag}${options.keys ? `?keys=${options.keys.join(',')}` : ""}${options.exclude ? `?exclude=${options.exclude.join(',')}` : ""}`)
            .set('auth', this.token)
     ...
     */
    async getVersion() {
        let { text } = await get('http://api.cr-api.com/version')
        return text;
    }
}
